...
#Asmita Gautam, February 24,2020
#This script is to process the data in a directory based on specific conditions set forth by the lab instructions. 

##Part 1
##Creating the HigherElevation directory and conferming it exists
if [ -d HigherElevation ] #if HigherElevation directory exists
then
...
##selecting the station altitude with 200 or more and saving output 
for file in StationData/*
do
  output=`basename "$file"`#basename is given a pathname, will delete any prefix up to the last slash ('/') character and return the result with their file name
  if 
  grep 'Station Altitude: [200,*]' $file #select stations equal to or over 200 altitude
...
convert -density 150x150 SoilMoistureStations.epsi SoilMoistureStations.tiff #convert .epsi to .tiff, dpi at 150
